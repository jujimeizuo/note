...
将$(x_1, x_2, ..., x_n)$看作是$n$维向量空间$R^n$中一个向量$\boldsymbol{x}$的$n$个分量，即$\boldsymbol{x} = (x_1, x_2, ..., x_n)^T$
...
多元函数$f(\boldsymbol{x})$的一阶导数是它的梯度$\bigtriangledown f(\boldsymbol{x})$，二阶导数是它的 Hesse 矩阵$\bigtriangledown^2f(\boldsymbol{x})$。
