...
2. 相似度含义
    - 有 n 个特征则组成 n 维向量 $X=[x_1, x_2, ..., x_n]^T$称为该样本的特征向量。它相当于特征空间中的一个点，以特征空间中，**点间的距离函数作为模式相似度的测量**，以“距离”作为模式分类的依据，**距离越小，越相似**
...
- **欧氏距离**
    ...
    设$X_1, X_2$为两个 n 维模式样本，$X_1 = [x_{11}, x_{12}, ..., x_{1n}]^T, X_2 = [x_{21}, x_{22}, ..., x_{2n}]^T$，欧氏距离定义为:

    $$
    D(X_1, X_2) = \parallel X_1 - X_2 \parallel = \sqrt{(X_1 - X_2)^T(X_1-X_2)}
    $$
...
- **马式距离**
    ...
    平方表达式：$D^2=(X_1-X_2)^TC^{-1}(X_1-X_2)$

    令$M:$均值向量 M_Mean，$C:$该类模式总体的协方差矩阵 C_covariance。

    对 n 维向量:$X=[x_1, ..., x_n]^T,M=[m_1,...,m_n]^T$
...
- **角度相似性函数**
    ...
    $$
    S(X_i, X_j) = \frac{X_i^TX_j}{\parallel X_i \parallel \cdot \parallel X_j \parallel}
    $$
...
- **Tanimoto 测度**
    ...
    $$
    S(X_i,X_j)=\frac{X_i^TX_j}{X_i^TX_i+X_j^TX_j-X_i^TX_j} \\
    \ \\
...
- $\omega_i \omega_i$两分法
    - 一个判别界面只能分开两个类别，不能把其余所有的类别都分开。判决函数为$d_{ij}(X)=W_{ij}^TX$
